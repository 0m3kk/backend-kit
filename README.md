...
1. **Clean Architecture & Trait Abstraction**: Core crates (`kv-store`, `event-sourcing`, `secret-store`, `email-sender`, `password-hasher`, `template-engine`) define pure async trait interfaces and domain models without heavy infrastructure dependencies.
