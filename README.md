...
1. **Clean Architecture & Trait Abstraction**: Core crates (`kv-store`, `event-sourcing`, `secret-store`, `email-sender`, `sms-sender`, `password-hasher`, `template-engine`, `two-factor-auth`) define pure async trait interfaces and domain models without heavy infrastructure dependencies.
