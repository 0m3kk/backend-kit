...
│                                   Backend Kit                                    │
└──────────────────────────────────────────────────────────────────────────────────┘
├── Event Sourcing & CQRS ──► event-sourcing, event-store-postgres, event-store-umadb, cqrs, tx-manager
...
1. **Clean Architecture & Trait Abstraction**: Core crates (`kv-store`, `event-sourcing`, `secret-store`, `email-sender`, `sms-sender`, `password-hasher`, `template-engine`, `two-factor-auth`, `webauthn`, `attempt-policy`, `tx-manager`) define pure async trait interfaces and domain models without heavy infrastructure dependencies.
...
| **CQRS**                | [`cqrs`](crates/cqrs/README.md)                                               | [`crates/cqrs`](crates/cqrs/README.md)                                               | 5-step Command pipeline (`Command<M>`), multi-model hydration (`DecisionModels`), self-managed transactions (`dispatch_command_in_tx`, `CommandDispatcherTx`), KV snapshots, object-safe views (`View<C>`), checkpoints (`KvCheckpointStore`), and background workers (`CatchupWorker`, `CatchupWorkerTx`). |
| **Transaction Manager** | [`tx-manager`](crates/tx-manager/README.md)                                   | [`crates/tx-manager`](crates/tx-manager/README.md)                                   | Unified transaction lifecycle abstraction (`TxContext`, `TransactionProvider`), `PostgresTxContext` (atomic multi-store handle across event store, KV, secret store), and automatic retry runner with exponential backoff on serialization failures.                                                        |
